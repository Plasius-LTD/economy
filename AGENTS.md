...
This repository contains `@plasius/economy`, a provider-neutral TypeScript
library for sitewide Token contracts and deterministic ledger invariants.
...
- Run `npm run lint`, `npm run typecheck`, `npm run build`,
  `npm run test:coverage`, and `npm run pack:check`.

### Package boundary

- Source belongs in `src/`; tests belong in `tests/`.
- Never add HTTP handlers, authentication middleware, provider SDKs, secrets,
  database clients, or Azure infrastructure to this package.
- Amounts use `bigint` internally and canonical base-10 strings at JSON
  boundaries. Floating-point arithmetic is forbidden.
- Public contracts must be versioned and backwards compatible where practical.
- Every posting set must remain exactly balanced and projection rebuilds must be
  deterministic.
- Do not edit generated `dist/` or `coverage/` output.
- Add TSDoc, tests, README/CHANGELOG updates, and flat ADRs under `docs/adrs/`
  for public or architectural changes.

### Safety

- Never include real PII, payment information, settlement evidence, provider
  callbacks, or secrets in source, tests, documentation examples, or logs.
- Publishing is GitHub CD-only; never run a local npm publish.
